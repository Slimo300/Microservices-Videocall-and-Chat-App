...
const Chat = ({group, user, ws, setCurrent}) => {
    ...
    const JoinCall = async (initialVideo, initialAudio) => {
        window.open("call/"+group.ID+"?initialAudio="+initialAudio+"&initialVideo="+initialVideo, "_blank", 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no');
    }
        ...
        load = (
            <div className="col-xl-8 col-lg-8 col-md-8 col-sm-9 col-9">
                <div className="selected-user row">
                    ...
                    <button className="btn btn-primary mt-3 mr-1 mb-3" type="button" onClick={() => JoinCall(false, true)}>
                        <FontAwesomeIcon icon={faPhone} />
                    </button>
                    <button className="btn btn-primary mt-3 mr-1 mb-3" type="button" onClick={() => JoinCall(true, true)}>
